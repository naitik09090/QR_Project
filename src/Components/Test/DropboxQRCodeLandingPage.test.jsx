...
import DropboxQRCodeLandingPage from '../Files & Tools/DropboxQRCodeLandingPage';
...
describe('DropboxQRCodeLandingPage Details', () => {

    test('DropboxQRCodeLandingPage renders correctly', () => {
        renderWithHelmet(<DropboxQRCodeLandingPage />);
        expect(screen.getByRole('heading', { level: 1 })).toHaveTextContent(/Dropbox to QR Code/i);
        expect(screen.getByLabelText(/Dropbox Share URL/i)).toBeInTheDocument();
    });

    test('DropboxQRCodeLandingPage has common elements', () => {
        renderWithHelmet(<DropboxQRCodeLandingPage />);
