...
import GooglePayQRCodeLandingPage from '../Payment_Qr_Pages/GooglePayQRCodeLandingPage';
...
describe('GooglePayQRCodeLandingPage Details', () => {

    test('GooglePayQRCodeLandingPage renders correctly', () => {
        renderWithHelmet(<GooglePayQRCodeLandingPage />);
        expect(screen.getByRole('heading', { level: 1 })).toHaveTextContent(/Google Pay QR Codes/i);
        expect(screen.getByLabelText(/UPI ID \/ VPA/i)).toBeInTheDocument();
    });

    test('GooglePayQRCodeLandingPage has common elements', () => {
        renderWithHelmet(<GooglePayQRCodeLandingPage />);
