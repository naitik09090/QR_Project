...
import InstagramProfileQRCodeLandingPage from '../Social Media/InstagramProfileQRCodeLandingPage';
...
describe('InstagramProfileQRCodeLandingPage Details', () => {

    test('InstagramProfileQRCodeLandingPage renders correctly', () => {
        renderWithHelmet(<InstagramProfileQRCodeLandingPage />);
        expect(screen.getByRole('heading', { level: 1 })).toHaveTextContent(/Instagram QR Codes/i);
        expect(screen.getByLabelText(/Username \/ Display Name/i)).toBeInTheDocument();
    });

    test('InstagramProfileQRCodeLandingPage has common elements', () => {
        renderWithHelmet(<InstagramProfileQRCodeLandingPage />);
    ...
    test('simulates user input and QR code generation', async () => {
        ...
        render(
            <HelmetProvider>
                <InstagramProfileQRCodeLandingPage />
            </HelmetProvider>
        );
