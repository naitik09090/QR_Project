...
import FlyersQRCodeLandingPage from '../Business & Marketing/FlyersQRCodeLandingPage';
...
describe('FlyersQRCodeLandingPage Details', () => {

    test('FlyersQRCodeLandingPage renders correctly', () => {
        renderWithHelmet(<FlyersQRCodeLandingPage />);
        expect(screen.getAllByText(/Flyers & Posters/i)[0]).toBeInTheDocument();
        expect(screen.getByLabelText(/Promo \/ Event URL/i)).toBeInTheDocument();
    });

    test('FlyersQRCodeLandingPage has common elements', () => {
        renderWithHelmet(<FlyersQRCodeLandingPage />);
