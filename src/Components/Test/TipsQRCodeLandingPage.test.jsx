...
import TipsQRCodeLandingPage from '../Payment_Qr_Pages/TipsQRCodeLandingPage';
...
describe('TipsQRCodeLandingPage Details', () => {

    test('TipsQRCodeLandingPage renders correctly', () => {
        renderWithHelmet(<TipsQRCodeLandingPage />);
        expect(screen.getByRole('heading', { level: 1 })).toHaveTextContent(/Tips QR Codes/i);
        expect(screen.getByLabelText(/Staff Name \/ ID/i)).toBeInTheDocument();
    });

    test('TipsQRCodeLandingPage has common elements', () => {
        renderWithHelmet(<TipsQRCodeLandingPage />);
