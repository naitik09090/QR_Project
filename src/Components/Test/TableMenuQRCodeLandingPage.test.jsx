...
import TableMenuQRCodeLandingPage from '../Restaurant & Hospitality/TableMenuQRCodeLandingPage';
...
describe('TableMenuQRCodeLandingPage Details', () => {

    test('TableMenuQRCodeLandingPage renders correctly', () => {
        renderWithHelmet(<TableMenuQRCodeLandingPage />);
        expect(screen.getByRole('heading', { level: 1 })).toHaveTextContent(/Table Menu QR/i);
        expect(screen.getByLabelText(/Restaurant Name/i)).toBeInTheDocument();
    });

    test('TableMenuQRCodeLandingPage has common elements', () => {
        renderWithHelmet(<TableMenuQRCodeLandingPage />);
