...
import FoodDeliveryMenuQRCodeLandingPage from '../Restaurant & Hospitality/FoodDeliveryMenuQRCodeLandingPage';
...
describe('FoodDeliveryMenuQRCodeLandingPage Details', () => {

    test('FoodDeliveryMenuQRCodeLandingPage renders correctly', () => {
        renderWithHelmet(<FoodDeliveryMenuQRCodeLandingPage />);
        expect(screen.getByRole('heading', { level: 1 })).toHaveTextContent(/Food Delivery QR/i);
        expect(screen.getByLabelText(/Restaurant Name/i)).toBeInTheDocument();
    });

    test('FoodDeliveryMenuQRCodeLandingPage has common elements', () => {
        renderWithHelmet(<FoodDeliveryMenuQRCodeLandingPage />);
    ...
    test('simulates user input and QR code generation', async () => {
        ...
        render(
            <HelmetProvider>
                <FoodDeliveryMenuQRCodeLandingPage />
            </HelmetProvider>
        );
