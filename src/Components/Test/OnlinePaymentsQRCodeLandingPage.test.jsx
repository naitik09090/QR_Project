...
import OnlinePaymentsQRCodeLandingPage from '../Payment_Qr_Pages/OnlinePaymentsQRCodeLandingPage';
...
describe('OnlinePaymentsQRCodeLandingPage Details', () => {

    test('OnlinePaymentsQRCodeLandingPage renders correctly', () => {
        renderWithHelmet(<OnlinePaymentsQRCodeLandingPage />);
        expect(screen.getByRole('heading', { level: 1 })).toHaveTextContent(/Online Payment QR Codes/i);
        expect(screen.getByLabelText(/Checkout URL \/ Payment Link/i)).toBeInTheDocument();
    });

    test('OnlinePaymentsQRCodeLandingPage has common elements', () => {
        renderWithHelmet(<OnlinePaymentsQRCodeLandingPage />);
    ...
    test('simulates user input and QR code generation', async () => {
        ...
        render(
            <HelmetProvider>
                <OnlinePaymentsQRCodeLandingPage />
            </HelmetProvider>
        );
