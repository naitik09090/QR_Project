...
import CafeMenuQRCodeLandingPage from '../Restaurant & Hospitality/CafeMenuQRCodeLandingPage';
...
describe('CafeMenuQRCodeLandingPage Details', () => {

    test('CafeMenuQRCodeLandingPage renders correctly', () => {
        renderWithHelmet(<CafeMenuQRCodeLandingPage />);
        expect(screen.getByRole('heading', { level: 1 })).toHaveTextContent(/Cafe QR Menus/i);
        expect(screen.getByLabelText(/Cafe or Coffee Shop Name/i)).toBeInTheDocument();
    });

    test('CafeMenuQRCodeLandingPage has common elements', () => {
        renderWithHelmet(<CafeMenuQRCodeLandingPage />);
