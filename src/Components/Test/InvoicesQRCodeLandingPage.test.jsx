...
import InvoicesQRCodeLandingPage from '../Payment_Qr_Pages/InvoicesQRCodeLandingPage';
...
describe('InvoicesQRCodeLandingPage Details', () => {

    test('InvoicesQRCodeLandingPage renders correctly', () => {
        renderWithHelmet(<InvoicesQRCodeLandingPage />);
        expect(screen.getByRole('heading', { level: 1 })).toHaveTextContent(/Invoice QR Codes/i);
        expect(screen.getByLabelText(/Invoice Number \/ ID/i)).toBeInTheDocument();
    });

    test('InvoicesQRCodeLandingPage has common elements', () => {
        renderWithHelmet(<InvoicesQRCodeLandingPage />);
