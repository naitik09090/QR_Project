...
import SmallBusinessQRCodeLandingPage from '../Business & Marketing/SmallBusinessQRCodeLandingPage';
...
describe('SmallBusinessQRCodeLandingPage Details', () => {

    test('SmallBusinessQRCodeLandingPage renders correctly', () => {
        renderWithHelmet(<SmallBusinessQRCodeLandingPage />);
        expect(screen.getAllByText(/Small Business/i)[0]).toBeInTheDocument();
        expect(screen.getByLabelText(/Destination URL/i)).toBeInTheDocument();
    });

    test('SmallBusinessQRCodeLandingPage has common elements', () => {
        renderWithHelmet(<SmallBusinessQRCodeLandingPage />);
    ...
    test('simulates user input and QR code generation', async () => {
        ...
        render(
            <HelmetProvider>
                <SmallBusinessQRCodeLandingPage />
            </HelmetProvider>
        );
