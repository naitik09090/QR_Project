...
import RoomServiceQRCodeLandingPage from '../Restaurant & Hospitality/RoomServiceQRCodeLandingPage';
...
describe('RoomServiceQRCodeLandingPage Details', () => {

    test('RoomServiceQRCodeLandingPage renders correctly', () => {
        renderWithHelmet(<RoomServiceQRCodeLandingPage />);
        expect(screen.getByRole('heading', { level: 1 })).toHaveTextContent(/Room Service QR/i);
        expect(screen.getByLabelText(/Hotel Name/i)).toBeInTheDocument();
    });

    test('RoomServiceQRCodeLandingPage has common elements', () => {
        renderWithHelmet(<RoomServiceQRCodeLandingPage />);
