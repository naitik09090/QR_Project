...
import ZipFileQRCodeLandingPage from '../Files & Tools/ZipFileQRCodeLandingPage';
...
describe('ZipFileQRCodeLandingPage Details', () => {

    test('ZipFileQRCodeLandingPage renders correctly', () => {
        renderWithHelmet(<ZipFileQRCodeLandingPage />);
        expect(screen.getByRole('heading', { level: 1 })).toHaveTextContent(/ZIP to QR Code/i);
        expect(screen.getByLabelText(/ZIP File URL/i)).toBeInTheDocument();
    });

    test('ZipFileQRCodeLandingPage has common elements', () => {
        renderWithHelmet(<ZipFileQRCodeLandingPage />);
