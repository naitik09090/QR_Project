...
import StripePaymentQRCodeLandingPage from '../Payment_Qr_Pages/StripePaymentQRCodeLandingPage';
...
describe('StripePaymentQRCodeLandingPage Details', () => {

    test('StripePaymentQRCodeLandingPage renders correctly', () => {
        renderWithHelmet(<StripePaymentQRCodeLandingPage />);
        expect(screen.getByRole('heading', { level: 1 })).toHaveTextContent(/Stripe Payment QR Codes/i);
        expect(screen.getByLabelText(/Stripe Payment Link URL/i)).toBeInTheDocument();
    });

    test('StripePaymentQRCodeLandingPage has common elements', () => {
        renderWithHelmet(<StripePaymentQRCodeLandingPage />);
    ...
    test('simulates user input and QR code generation', async () => {
        ...
        render(
            <HelmetProvider>
                <StripePaymentQRCodeLandingPage />
            </HelmetProvider>
        );
