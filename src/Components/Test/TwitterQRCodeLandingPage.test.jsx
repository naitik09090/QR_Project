...
import TwitterQRCodeLandingPage from '../Social Media/TwitterQRCodeLandingPage';
...
describe('TwitterQRCodeLandingPage Details', () => {

    test('TwitterQRCodeLandingPage renders correctly', () => {
        renderWithHelmet(<TwitterQRCodeLandingPage />);
        expect(screen.getByRole('heading', { level: 1 })).toHaveTextContent(/Twitter \(X\) QR Codes/i);
        expect(screen.getByLabelText(/Profile, Tweet, or Hashtag Link/i)).toBeInTheDocument();
    });

    test('TwitterQRCodeLandingPage has common elements', () => {
        renderWithHelmet(<TwitterQRCodeLandingPage />);
