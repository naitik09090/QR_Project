...
import DonationsQRCodeLandingPage from '../Payment_Qr_Pages/DonationsQRCodeLandingPage';
...
describe('DonationsQRCodeLandingPage Details', () => {

    test('DonationsQRCodeLandingPage renders correctly', () => {
        renderWithHelmet(<DonationsQRCodeLandingPage />);
        expect(screen.getByRole('heading', { level: 1 })).toHaveTextContent(/Donation QR Codes/i);
        expect(screen.getByLabelText(/Donation Page \/ Fundraising URL/i)).toBeInTheDocument();
    });

    test('DonationsQRCodeLandingPage has common elements', () => {
        renderWithHelmet(<DonationsQRCodeLandingPage />);
